...
class DiffMixer(nn.Module):
    ...
    def diff(self, x: torch.Tensor) -> torch.Tensor:
        x = np.diff(x.cpu().detach().numpy(), n=1, axis=2)
...
class DiffFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.blocks[-1](x).transpose(1, 2)

        x = F.cosine_similarity(x.unsqueeze(2), x.unsqueeze(1), dim=-1)  # (b*nw, nf, nf, nl*3)
...
if __name__ == '__main__':
    ...
    model = DiffFormer(dim=256, diff_idx=0, num_blocks=1)
    out = model(x)
    print(out.shape)
