...
class STQIHead(DIIHead):
    ...
    def forward(self, roi_feat, proposal_feat, clip_length):
        ...
        # temporal_context of roi_feat
        # roi_feat = self.tsm_label(roi_feat)
        # print('bbox_feats2:', bbox_feats.shape)

        # roi_feat = roi_feat.reshape(-1, clip_length, roi_feat.size(-3), roi_feat.size(-2), roi_feat.size(-1))
        # roi_feat = roi_feat.transpose(1, 2).contiguous()
        # roi_feat = self.temporal_context().to(device)(roi_feat)
        # roi_feat = roi_feat.transpose(1, 2).contiguous()
        # roi_feat = roi_feat.reshape(-1, roi_feat.size(-3), roi_feat.size(-2), roi_feat.size(-1))

        # roi_feat = roi_feat.transpose(1, 2).contiguous()
        # roi_feat = roi_feat.reshape(-1, roi_feat.size(-3), roi_feat.size(-2), roi_feat.size(-1))
        # print('bbox_feats4:', bbox_feats.shape)
        # roi_feat = self.tsm_label_revise(roi_feat)
        # print('bbox_feats5:', bbox_feats.shape)
        ...
        # print('roi_feat_before_ffn:', roi_feat.shape)
        # print('proposal_feat_before_ffn:', proposal_feat.shape)

        obj_feat = self.instance_interactive_conv_norm(proposal_feat)  # a layer norm
        # print('obj_feat_before_ffn:', obj_feat.shape)
