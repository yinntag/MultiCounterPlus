...
class MultiRepEval:
    # Interface for evaluating on the MultiRep dataset.
    # Modified from YouTubeVIS API (https://github.com/youtubevos/cocoapi)

    # The usage for MultiRepEval is as follows:
    ...
    def _prepare(self):
        ...
        if p.useCats:
            gts = self.cocoGt.loadAnns(self.cocoGt.getAnnIds(vidIds=p.vidIds,
                                                             catIds=p.catIds))  # Each element in gts represents an gt instance
            dts = self.cocoDt.loadAnns(self.cocoDt.getAnnIds(vidIds=p.vidIds,
                                                             catIds=p.catIds))  # Each element in dts represents an predicted instance
        else:
            gts = self.cocoGt.loadAnns(self.cocoGt.getAnnIds(vidIds=p.vidIds))
            dts = self.cocoDt.loadAnns(self.cocoDt.getAnnIds(vidIds=p.vidIds))
    ...
    def computeIoU(self, vidId, catId):
        ...
        dt = [dt[i] for i in
              inds]  # Sort the confidence of each prediction dt of the current class of the current video from largest to smallest according to the sorting index obtained in the previous line
        ...
        for i, j in np.ndindex(ious.shape):
            ious[i, j] = iou_seq(d[i],
                                 g[j])  # Calculate the spatio-temporal iou for the specified pred and gt in a video
        return ious
    ...
    def evaluateVid(self, vidId, catId, aRng, maxDet):
        ...
        dtIg = np.zeros((T, D))
        if not len(
                ious) == 0:  # Determine the matching relationship between gt and pred, which is same as temporal AP in ActivityNet
            for tind, t in enumerate(p.iouThrs):  # The IoU threshold is traversed, 0.5:0.95 for a total of 10 values
                for dind, d in enumerate(dt):
                    ...
                    for gind, g in enumerate(gt):
                        ...
                        iou = ious[
                            dind, gind]  # Here iou will be updated to the current maximum iou, so after the loop, the item with the maximum iou will be matched
                        m = gind
                    ...
                    dtm[tind, dind] = gt[m][
                        'id']  # The 'dind' pred matches the 'm' (m=gind) gt, now record the index of the gt it matches
                    gtm[tind, m] = d[
                        'id']  # The gt with index 'gind' is matched to pred, now record the index of its matched pred
        ...
        return {
            ...
            'dtMatches': dtm,
            # Two-dimensional array, each row represents an iou threshold, each element in a row corresponds to each dt (after sorting), its value is the matched gt id, this id is global (the id of all samples), 0 is no match to gt
            'gtMatches': gtm,
            # Two-dimensional array, each row represents an iou threshold, each element within a row corresponds to each gt (after sorting), its value is the matched dt id, this id is also global, 0 is no match to the requested dt
            ...
            for a, a0 in enumerate(a_list):
                ...
                for iou in range(0, 10):
                    ...
                    for gt_index in range(0, len(gt_match_info[iou])):
                        ...
                        gt_collection.append(
                            {'gt_ID': real_gt_id, 'periodicity': self.cocoGt.loadAnns(real_gt_id)[0]['periodicity']})
                        dt_collection.append(
                            {'gt_ID': real_gt_id, 'periodicity': self.cocoDt.loadAnns(matched_dt_id)[0]['periods_converted']})
                    self.periodicity_eval_info.append({'iou': iou_type, 'areaRng': _pe.areaRng[a0], 'dt_data': dt_collection,
                                                 'gt_data': gt_collection})
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    dtScores = np.concatenate(
                        [e['dtScores'][0:maxDet] for e in E])  # topK confidence pred is taken out for each video

                    # different sorting method generates slightly different results.
                    # mergesort is used to be consistent as Matlab implementation.
                    inds = np.argsort(-dtScores, kind='mergesort')
                    dtScoresSorted = dtScores[
                        inds]  # pred does not distinguish between video, arranged in descending order
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):  # for each iou threshold
                        ...
                        try:
                            for ri, pi in enumerate(inds):
                                ...
                                ss[ri] = dtScoresSorted[
                                    pi]  # get the confidence level of each point corresponding to the pr curve
                        except:
                            pass
    ...
    def action_ap(self):
        index = 0
        for config in self.periodicity_eval_info:
            index += 1
            if index > 1:
                break
            gt = np.empty(shape=[0, 3], dtype=int)
            for instance in config['gt_data']:
                for periodicity_event in instance['periodicity']:
                    gt = np.append(gt, [[instance['gt_ID'], periodicity_event[0], periodicity_event[1]]], axis=0)
            dt = np.empty(shape=[0, 4], dtype=float)
            for instance in config['dt_data']:
                for periodicity_event in instance['periodicity']:
                    dt = np.append(dt, [[instance['gt_ID'], periodicity_event[0], periodicity_event[1], periodicity_event[2]]], axis=0)
            ...
            print(f'Instance-level IoU={config["iou"]}, area@{config["areaRng"]}, Period-AP@0.5={ap[0]}')
            print(f'Instance-level IoU={config["iou"]}, area@{config["areaRng"]}, Period-AP@0.75={ap[5]}')
            print(f'Instance-level IoU={config["iou"]}, area@{config["areaRng"]}, Period-AP@0.95={ap[-1]}')
            print(f'Instance-level IoU={config["iou"]}, area@{config["areaRng"]}, Period-AP@0.5:0.95={ap.mean()}')
    ...
    def compute_average_precision_detection(self, ground_truth, prediction, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
        ...
        lock_gt = np.ones(
            (len(tiou_thresholds), len(ground_truth))) * -1  # [num_tiou, num_gt] Initial values are all -1
        # Sort predictions by decreasing score order.
        sort_idx = prediction['score'].values.argsort()[::-1]
        prediction = prediction.loc[sort_idx].reset_index(
            drop=True)  # Sorted by confidence from highest to lowest, not distinguishing video_id
            ...
            for tidx, tiou_thr in enumerate(tiou_thresholds):
                for jdx in tiou_sorted_idx:  # Traverse in order of tIoU from largest to smallest
                    if tiou_arr[jdx] < tiou_thr:
                        ...
                        break
                    if lock_gt[
                        tidx, this_gt.loc[jdx][
                            'index']] >= 0:  # If it is matched, then continue and see if the next gt with a slightly smaller tIoU can meet the threshold condition and has not been matched.
                        continue
                    # Assign as true positive after the filters above.
                    tp[tidx, idx] = 1  # Under this iou_threshold, this sample is TP
                    lock_gt[tidx, this_gt.loc[jdx][
                        'index']] = idx  # Under this iou_threshold, lock this gt, which means it has been matched
                    break

                if fp[tidx, idx] == 0 and tp[
                    tidx, idx] == 0:  # If a gt meets the threshold condition but has already been matched, the prediction is still FP
                    fp[tidx, idx] = 1
