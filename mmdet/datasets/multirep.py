...
from .multirep_api import MultiRep
from .builder import DATASETS
from .custom import CustomDataset
from .pipelines import Compose
from .label_to_dm import preprocess


@DATASETS.register_module()
class MultiRepDataset(CustomDataset):
    ...
    def load_annotations(self, ann_file):
        self.multirep = MultiRep(ann_file)   # multirep api to read gt json file
        self.cat_ids = self.multirep.getCatIds()
        self.cat2label = {cat_id: i for i, cat_id in enumerate(self.cat_ids)}  # Mapping relationship between categories and labels
        vid_ids = self.multirep.getVidIds()  # a list of all videos
        vid_infos = []
        for i in vid_ids:
            info = self.multirep.loadVids([i])[0]  # Load the basic information of the video with the current video id
    ...
    def _filter_imgs(self, min_size=32):
        ...
        if self.filter_empty_gt:
            for i, (vid, frame_id) in enumerate(self.data_infos):
                ...
                ann_ids = self.multirep.getAnnIds(vidIds=[vid_id])  # return anno ids within this vid_id
                ann_info = self.multirep.loadAnns(ann_ids)  # obtain annotations based on the anno ids
                # print(ann_info[0]['video_id'])
                # print(frame_id)
                anns = [
    ...
    def get_ann_info(self, idx):
        vid, frame_id = self.data_infos[idx]
        vid_id = self.vid_infos[vid]['id']
        ann_ids = self.multirep.getAnnIds(vidIds=[vid_id])
        ann_info = self.multirep.loadAnns(ann_ids)
    ...
    def _parse_ann_info(self, ann_info, frame_id):
        """Parse bbox and periodicity annotation.
        ...
        for i, ann in enumerate(ann_info):  # for each instance
            ...
            # print('frame_id2:', frame_id)
            # area = ann['areas'][frame_id]
            # if bbox is None:
            # if bbox == [0.0, 0.0, 0.0, 0.0]:  # follow multirep to devise
            #     continue
            x1, y1, w, h = bbox
            ...
            if ann.get('iscrowd', False):
                ...
                # print('sampling_rate2: ', self.frame_interval)
                gt_bboxes.append(bbox)
                gt_ids.append(ann['id'] - 1)  # multirep instance id start from 1. 1 --> 0
                gt_labels.append(self.cat2label[ann['category_id']])
                gt_periods.append(ann['periods'][frame_id] / self.frame_interval)   # 得到一帧里面所有object的label信息, 按照采样率将对应的高斯分数翻倍
                gt_periodicity.append(ann['periodicity_binary'][frame_id])
        # print('='*100)
        if gt_bboxes:
            gt_bboxes = np.array(gt_bboxes, dtype=np.float32)  # list --> numpy array
            gt_labels = np.array(gt_labels, dtype=np.int64)
            gt_periods = np.array(gt_periods, dtype=np.int64)
            gt_periodicity = np.array(gt_periodicity, dtype=np.int64)
        else:
    ...
    def check_data_rep(self, data):
        # all values in tensors < 64
        all_values_less_than_200 = all(torch.all(data_tensor < 64) for data_tensor in data)

        # all sums >= 2
        sums_of_reciprocals = []
        for i in range(len(data[0])):
            reciprocal_sum = sum(1 / data_tensor[i] for data_tensor in data if data_tensor[i] != 0)
            sums_of_reciprocals.append(reciprocal_sum)

        all_reciprocals_greater_than_2 = all(s > 2 for s in sums_of_reciprocals)

        return all_values_less_than_200 and all_reciprocals_greater_than_2
        # return all_values_less_than_200

    def check_data_dm(self, data):
        # all sums >= 2
        sums_of_reciprocals = []
        for i in range(len(data[0])):
            reciprocal_sum = sum(data_tensor[i] for data_tensor in data if data_tensor[i] != 0)
            sums_of_reciprocals.append(reciprocal_sum)

        all_reciprocals_greater_than_2 = all(s > 2 for s in sums_of_reciprocals)

        return all_reciprocals_greater_than_2
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            filter_data = self.check_data_rep(data['gt_periods'].data)
    ...
    def prepare_train_clip(self, idx):
        vid, frame_id = self.data_infos[idx]
        vid_info = self.vid_infos[vid]
        sample_range = range(len(vid_info['filenames']))  # range(0, 299)
        valid_idxs = []
        ...
        self.frame_interval = random.randint(2, 6)
        pre = self.frame_interval*self.clip_length//2 - 1
        post = len(vid_info['filenames'])-self.frame_interval*self.clip_length//2 - 1
        new_frame_id = random.randint(min(pre, post), max(pre, post))
        index_pre = [(vid, new_frame_id - self.frame_interval*i) for i in range(1, self.clip_length//2 + 1) if (new_frame_id - self.frame_interval*i) >= valid_idxs[0][1] and (vid, new_frame_id - self.frame_interval*i) in valid_idxs]
        pre_res = [(vid, valid_idxs[0][1]) for i in range(0, self.clip_length//2 - len(index_pre))]
        index_pre = index_pre + pre_res
        index_post = [(vid, new_frame_id + self.frame_interval*i) for i in range(1, self.clip_length//2 + 1) if (new_frame_id + self.frame_interval*i) <= valid_idxs[-1][1] and (vid, new_frame_id + self.frame_interval*i) in valid_idxs]
        ...
        clip = []
        for _ in valid_idxs:
            clip.append(self.prepare_train_img(_))  # prepare_train_img(img and anno) frame-by-frame
